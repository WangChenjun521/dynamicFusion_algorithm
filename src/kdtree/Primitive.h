...
namespace ldp
{
	namespace kdtree
	{
		...
		public:
			...
			vector3& GetCentre() { return sphere.m_Centre; }
			real GetSqRadius() { return sphere.m_SqRadius; }
			real GetRadius() { return sphere.m_Radius; }
			// triangle primitive methods
			vector3 GetFaceNormal() { return triangle.m_faceNormal; }
			vector3* GetVertex(int a_Idx) { return triangle.m_Vertex[a_Idx]; }
			vector3* GetNormal(int a_Idx)
			{
				if (triangle.m_Normal[a_Idx] == 0)
					return &triangle.m_faceNormal;
				return triangle.m_Normal[a_Idx];
			}
			Vec2* GetTexCoord(int a_Idx) { return triangle.m_texCoords[a_Idx]; }
			void SetVertex(int a_Idx, vector3* a_Vertex) { triangle.m_Vertex[a_Idx] = a_Vertex; }
			void SetNormal(int a_Idx, vector3* a_Normal) {triangle. m_Normal[a_Idx] = a_Normal; }
			void SetTexCoord(int a_Idx, Vec2* a_tex) { triangle.m_texCoords[a_Idx] = a_tex; }
			...
			FINLINE AABB getAABB()const
			{
				if (m_Type == SPHERE)
				{
					return AABB(sphere.m_Centre - sphere.m_Radius, sphere.m_Centre + sphere.m_Radius);
				}
				else
				{
					AABB ab(*triangle.m_Vertex[0]);
					for (int k = 0; k < 3; k++)
					{
						ab.min[k] = std::min(ab.min[k], (*triangle.m_Vertex[0])[k]);
						ab.max[k] = std::max(ab.max[k], (*triangle.m_Vertex[0])[k]);
						ab.min[k] = std::min(ab.min[k], (*triangle.m_Vertex[1])[k]);
						ab.max[k] = std::max(ab.max[k], (*triangle.m_Vertex[1])[k]);
						ab.min[k] = std::min(ab.min[k], (*triangle.m_Vertex[2])[k]);
						ab.max[k] = std::max(ab.max[k], (*triangle.m_Vertex[2])[k]);
					}
					return ab;
				}
			}
