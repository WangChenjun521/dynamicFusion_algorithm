...
namespace ldp
{
	namespace kdtree
	{
		...
		Primitive::Primitive(int a_Type, vector3& a_Centre, real a_Radius) :Primitive()
		{
			...
			sphere.m_Centre = a_Centre;
			sphere.m_SqRadius = a_Radius * a_Radius;
			sphere.m_Radius = a_Radius;
			sphere.m_RRadius = 1.0f / a_Radius;
			m_Type = a_Type;
			m_Material = NULL;
			// set vectors for texture mapping
			sphere.m_Vn = vector3(0, 1, 0);
			sphere.m_Ve = vector3(1, 0, 0);
			sphere.m_Vc = sphere.m_Vn.cross(sphere.m_Ve);
		}
		...
		Primitive::Primitive(int a_Type, vector3* a_V1, vector3* a_V2, vector3* a_V3) :Primitive()
		{
			m_Type = a_Type;
			m_Material = 0;
			triangle.m_Vertex[0] = a_V1;
			triangle.m_Vertex[1] = a_V2;
			triangle.m_Vertex[2] = a_V3;
			// init precomp
			vector3 A = *triangle.m_Vertex[0];
			vector3 B = *triangle.m_Vertex[1];
			vector3 C = *triangle.m_Vertex[2];
			vector3 c = B - A;
			vector3 b = C - A;
			triangle.m_faceNormal = c.cross(b);
			...
			for (int j = 0; j<3; j++) {
				if (std::abs(triangle.m_faceNormal[j]) > std::abs(triangle.m_faceNormal[k]))
					k = j;
			}

			uint32_t u = waldModulo[k],
				v = waldModulo[k + 1];
			const Float n_k = triangle.m_faceNormal[k],
				denom = b[u] * c[v] - b[v] * c[u];

			assert(denom != 0);

			/* Pre-compute intersection calculation constants */
			triangle.n_u = triangle.m_faceNormal[u] / n_k;
			triangle.n_v =triangle.m_faceNormal[v] / n_k;
			triangle.n_d = (*a_V1).dot(triangle.m_faceNormal) / n_k;
			triangle.b_nu = b[u] / denom;
			triangle.b_nv = -b[v] / denom;
			triangle.a_u = A[u];
			triangle.a_v = A[v];
			triangle.c_nu = c[v] / denom;
			triangle.c_nv = -c[u] / denom;

			// finalize normal
			triangle.m_faceNormal.normalizeLocal();

			triangle.m_Normal[0] = 0;
			triangle.m_Normal[1] = 0;
			triangle.m_Normal[2] = 0;

			triangle.m_texCoords[0] = 0;
			triangle.m_texCoords[1] = 0;
			triangle.m_texCoords[2] = 0;
		}
			...
			Vec2* a_T1, Vec2* a_T2, Vec2* a_T3) :Primitive(a_Type, a_V1, a_V2, a_V3)
		{
			...
			if (a_N1 != NULL)
				triangle.m_Normal[0] = a_N1;
			if (a_N2 != NULL)
				triangle.m_Normal[1] = a_N2;
			if (a_N3 != NULL)
				triangle.m_Normal[2] = a_N3;

			if (a_T1 != NULL)
				triangle.m_texCoords[0] = a_T1;
			if (a_T2 != NULL)
				triangle.m_texCoords[1] = a_T2;
			if (a_T3 != NULL)
				triangle.m_texCoords[2] = a_T3;
		}
		...
		int Primitive::Intersect(const Ray& a_Ray, real& a_Dist, int thread_id)
		{
			if (m_Type == SPHERE)
			{
				vector3 v = a_Ray.o - sphere.m_Centre;
				real b = -DOT(v, a_Ray.d);
				real c = DOT(v, v) - sphere.m_SqRadius;
				real det = b*b - c;
				int retval = MISS;
				if (det > 0)
				{
			...
			else
			{
				...
				real t = (triangle.n_d - o_u*triangle.n_u - o_v*triangle.n_v - o_k) /
					(d_u * triangle.n_u + d_v * triangle.n_v + d_k);

				if (t < 0 || t > a_Dist) return MISS;

				/* Calculate the projected plane intersection point */
				const Float hu = o_u + t * d_u - triangle.a_u;
				const Float hv = o_v + t * d_v - triangle.a_v;

				/* In barycentric coordinates */
				real u = triangle.m_U[thread_id] = hv * triangle.b_nu + hu * triangle.b_nv;
				real v = triangle.m_V[thread_id] = hu * triangle.c_nu + hv * triangle.c_nv;
				if (!(u >= 0 && v >= 0 && u + v <= 1.0f)) return MISS;

				a_Dist = t;
				return (DOT(a_Ray.d, triangle.m_faceNormal) > 0) ? INPRIM : HIT;
			}
		}
		...
		vector3 Primitive::GetNormal(vector3& a_Pos, int thread_id)
		{
			if (m_Type == SPHERE)
			{
				return (a_Pos - sphere.m_Centre) * sphere.m_RRadius;
			}
			else
			{
				vector3 N1, N2, N3;
				if (triangle.m_Normal[0] == 0)
					N1 = N2 = N3 = triangle.m_faceNormal;
				else
				{
					N1 = *(triangle.m_Normal[0]);
					N2 = *(triangle.m_Normal[1]);
					N3 = *(triangle.m_Normal[2]);
				}
				vector3 N = N1 + triangle.m_U[thread_id] * (N2 - N1) + triangle.m_V[thread_id] * (N3 - N1);
				NORMALIZE(N);
				return N;
			}
		}
		...
		Color Primitive::GetColor(vector3& a_Pos, int thread_id)
		{
			...
			if (!m_Material->GetTexture()) retval = m_Material->GetColor(); else
			{
				if (m_Type == SPHERE)
				{
					vector3 vp = (a_Pos - sphere.m_Centre) * sphere.m_RRadius;
					real phi = acos(-DOT(vp, sphere.m_Vn));
					real u, v = phi * m_Material->GetVScaleReci() * (1.0f / PI);
					real theta = (acos(DOT(sphere.m_Ve, vp) / sin(phi))) * (2.0f / PI);
					if (DOT(sphere.m_Vc, vp) >= 0) u = (1.0f - theta) * m_Material->GetUScaleReci();
					else u = theta * m_Material->GetUScaleReci();
					retval = m_Material->GetTexture()->GetTexel(u, v) * m_Material->GetColor();
				}
				else
				{
					if (triangle.m_texCoords[0] != NULL && triangle.m_texCoords[1] != NULL && triangle.m_texCoords[2] != NULL)
					{
						Vec2 uv[3];
						for (int k = 0; k < 3; k++)
							uv[k] = *triangle.m_texCoords[k];
						real u = uv[0][0] + triangle.m_U[thread_id] * (uv[1][0] - uv[0][0]) + triangle.m_V[thread_id] * (uv[2][0] - uv[0][0]);
						real v = uv[0][1] + triangle.m_U[thread_id] * (uv[1][1] - uv[0][1]) +triangle. m_V[thread_id] * (uv[2][1] - uv[0][1]);
						retval = m_Material->GetTexture()->GetTexel(u, v) * m_Material->GetColor();
					}
					else
					{
...
#define AXISTEST_Z0( a, b, fa, fb )												\
	...
	rad = fa * a_BoxHalfsize[0] + fb * a_BoxHalfsize[1];				\
		...
		bool Primitive::IntersectBox( AABB&  a_Box)
		{
			if (m_Type == SPHERE)
			{
				return IntersectSphereBox(sphere.m_Centre, a_Box);
			}
			else
			{
				vector3 bpos = a_Box.getCorner(0);
				vector3 bsize = a_Box.getExtents();
				// vector3 times_0_5=vector3(0.5,0.5,0.5);
				bsize= bsize*0.5f;
				return IntersectTriBox( bpos, bsize,
					*triangle.m_Vertex[0], *triangle.m_Vertex[1], *triangle.m_Vertex[2]);
			}
		}
		...
		void Primitive::CalculateRange(real& a_Pos1, real& a_Pos2, int a_Axis)
		{
			if (m_Type == SPHERE)
			{
				a_Pos1 = sphere.m_Centre[a_Axis] - sphere.m_Radius;
				a_Pos2 = sphere.m_Centre[a_Axis] + sphere.m_Radius;
			}
			else
			{
				vector3 pos1 = *triangle.m_Vertex[0];
				a_Pos1 = pos1[a_Axis], a_Pos2 = pos1[a_Axis];
				for (int i = 1; i < 3; i++)
				{
					vector3 pos = *triangle.m_Vertex[i];
