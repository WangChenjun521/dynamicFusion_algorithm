	...
	public:
		...
		pointer allocate(size_type num, const void* hint = 0)
		{
			if (num > size_t(-1) / sizeof(T))
				throw std::bad_alloc();
			return static_cast<pointer>(std::aligned_alloc(num * sizeof(T), 16));
		}
...
#undef max
	...
	// 		gettimeofday(&startTime, &m_iFrequency);
	// 		gettimeofday(&endTime, &m_iFrequency);
	// 		//startTime.tv_sec // number of seconds since some epochstartTime.tv_usec 
	// 		// number of microseconds since last second. 	
	// 		pFile = nullptr;
	// 		Reset();
	// 	}
